...
CODEOWNERS = ["@ojschumann"]
DEPENDENCIES = ["spi"]

STMPE610_ns = cg.esphome_ns.namespace("stmpe610")
STMPE610Component = STMPE610_ns.class_(
    "STMPE610Component",
...
CONFIG_SCHEMA = cv.All(
    ...
    .extend(
        cv.Schema(
            {
                cv.GenerateID(): cv.declare_id(STMPE610Component),
                cv.Optional(CONF_INTERRUPT_PIN): cv.All(
                    pins.internal_gpio_input_pin_schema
                ),
                cv.Optional(CONF_THRESHOLD, default=128): cv.int_range(min=0, max=255),
            },
        )
    )
